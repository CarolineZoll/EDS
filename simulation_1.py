import eventDrivenSimulation_1 as eds
import simpy
import numpy as np
import pandas as pd
import scipy
...
def run_simulation(mode,df,cluster,mu, ue_nr, thr, max_prb,prb_number_comp,metric,sched_mode):
    ...
    for i in cluster:
        ...
        env.process(sched_l[counter].scheduler(env,ue_sep,SCHEDULE_T,cluster,max_prb,ue_list,prb_number_normal,metric,sched_mode))
        counter=counter+1
    timer=2000
    env.run(until=timer)
