...
class sched_inst:
    ...
    def __init__(self,env,cluster):
        ...
        self.rem_prb_c={}
        self.rem_req_c={}
        self.add_resources=np.array([])
        for i in cluster:
            self.add_resources=np.append(self.add_resources,simpy.Container(env))
            self.add_resources=np.append(self.add_resources,simpy.Container(env))
            
        self.scheduling_list_next_slot=np.array([])
        self.scheduling_list_next_slot_c=np.array([])
        
        
    def metric_list_nC(self, users,sched_exp,sched_mode,counter):
        e1=sched_exp[0]
        e2=sched_exp[1]
        metric=np.array([])
        if(sched_mode=='pf'):
            ...
            if(len(self.scheduling_list_next_slot)==0):
                sched_user_list=np.arange(0,len(users))
            else:
                sched_user_list =self.scheduling_list_next_slot
        return sched_user_list

    def metric_list_C(self, users,sched_exp,sched_mode,counter,usage):
        e1=sched_exp[0]
        e2=sched_exp[1]
        metric=np.array([])
        if(sched_mode=='pf'):
            ...
            if(len(self.scheduling_list_next_slot_c)==0):
                sched_user_list=np.arange(0,len(users))
            else:
                sched_user_list =self.scheduling_list_next_slot_c
        return sched_user_list


    def central_scheduler(self, env, users, SCHEDULE_T, cluster, prb_number,sched_metric,mode,sched_mode):
        
        alpha=-np.log10(0.01)/100
        while True: 
            #self.add_resources[0]=10
            #print(self.add_resources)
            ...
            sched_user_list=self.metric_list_C(users,sched_metric,sched_mode,env.now,'comp')
            remaining_prb_list=prb_number.copy()
            
            
            nr=0
            for z in cluster:
                remaining_prb_list[z]=remaining_prb_list[z]+self.add_resources[nr].level
                if(self.add_resources[nr].level>0):
                    self.add_resources[nr].get(self.add_resources[nr].level)
                nr+=1
            k=0
            free_res=1
            
            while(free_res==1):
                print(users[0].queue2.level)
                if(k==len(sched_user_list)):
                    print('happend')
                    nr=len(cluster)
                    for z in remaining_prb_list:
                        if(remaining_prb_list[z]>0):
                            self.add_resources[nr].put(remaining_prb_list[z])
                            print('central scheduler2',self.add_resources[nr].level)
                        nr+=1
                    break
                ...
                sched_size=0
                if(remaining_prbs==0): #wenn keine resourcen mehr frei sind
                    k+=1
                    continue
                ...
                users[sched_user].bits2+=sched_size
                ...
                self.scheduling_list_next_slot=sched_user_list.copy()
                save=self.scheduling_list_next_slot[0]
                self.scheduling_list_next_slot=np.delete(self.scheduling_list_next_slot,0)
                self.scheduling_list_next_slot=np.append(self.scheduling_list_next_slot,save)
    ...
    def scheduler(self, env, users, SCHEDULE_T,cluster, prb_number, users2,prb_number2, sched_metric,sched_mode, cs):
        ...
        while True: #größte Warteschlange wird auch bedient
            ...
            sched_user_list=self.metric_list_nC(users,sched_metric,sched_mode,counter)
            ...
            sched_user_list = self.metric_list_C(users2,sched_metric,sched_mode,counter,'nocomp') #calculates the ordered list with ues
            cell_number=int(users2[0].cell1)
            index_give=0
            index_get=0
            
            for i in np.arange(0,len(cluster)):
                if(cluster[i]== cell_number):
                    index_give=i
            index_get=index_give+len(cluster)
            ...
            while(remaining_prbs>0):
                if(k==len(sched_user_list)):
                    #print('remaining res comp1:',remaining_prbs)
                    cs.add_resources[index_give].put(remaining_prbs)
                    #print('TP scheduler',cs.add_resources[index_give].level)
                    break
                sched_user=sched_user_list[k]
                ...
                if((queue_size/tbs)<=remaining_prbs and queue_size>0):
                    ...
                    cs.add_resources[index_give].put(remaining_prbs)
                    #print('TP scheduler2',cs.add_resources[index_give].level)
                    break
                else:
                    print('something went wrong')
                ...
                self.scheduling_list_next_slot_c=sched_user_list.copy()
                save=self.scheduling_list_next_slot_c[0]
                self.scheduling_list_next_slot_c=np.delete(self.scheduling_list_next_slot_c,0)
                self.scheduling_list_next_slot_c=np.append(self.scheduling_list_next_slot_c,save)
            ###########################
